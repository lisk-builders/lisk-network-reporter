...
var MAX_CONNECTION_ATTEMPTS = 5000;
	...
	var options = {
  	...
  	res.setEncoding('utf8');
  		res.on('data', function (data) {
       		...
       		//console.info('Response:', jsonObject);
       		if (action == 'check') {
       			...
       			self.info.coinbase = "Lisk " + jsonObject["version"] + " Build:(" +jsonObject["build"]+")";
				self.info.node = "Lisk " + jsonObject["version"] + " Build:(" +jsonObject["build"]+")";
				...
				//Should be replace with reasonable endpoint /count/ as soon will be added
				var peer_list = jsonObject["peers"];
				var peer_count = peer_list.length;
...
Node.prototype.checkConnection = function() {
	var self = this;
	self.askLisk('GET','/api/loader/status/sync','check', '', self);
	if(isConnected) {
...
Node.prototype.startSocketConnection = function() {
	if( !this._socket ){
		...
		socket = new Socket( process.env.WS_SERVER || 'wss://liskstats.net');
		this.setupSockets();
	}
}
...
Node.prototype.getInfo = function() {
	...
	self.askLisk('GET','/api/peers/version','nodeinfo', '', self);
	return true;
}
...
Node.prototype.validateLatestBlock = function (error, result, timeString, self) {
	...
	block.transactions = result['numberOfTransactions'];
	...
	console.info('BlockNumber:',this.stats.block.number);
...
Node.prototype.getStats = function(forced) {
	...
	if (lastFetchAgo >= UPDATE_INTERVAL || forced === true) {
		...
		//Temporary checking if node is forging form config, as soon Lisk adds reasonable endpoint for this should be check with Lisk if acually is forging
		...
		if (STATS_ITERATION == 15) {
			self.getInfo();
			STATS_ITERATION = 0;
		}
		STATS_ITERATION++;
		self.getPending();
		self.askLisk('GET','/api/delegates/count','delegates', '', self);
		self.askLisk('GET','/api/peers?state=2&limit=100&offset=0','peers', '', self);
		self.getLatestBlock();
	}
}
...
Node.prototype.getPending = function() {
	...
	self.askLisk('GET','/api/transactions/unconfirmed','pendingtx', now, self);
	return false;	
}
...
Node.prototype.setWatches = function() {
	...
	self.askLisk('GET','/api/loader/status/sync','syncing', '', self);
}
