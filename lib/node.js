		...
		res.on('data', function (chunk) {
  		...
  		res.on('end', function () {
       		...
       		if (action == 'check') {
       			...
       			var NodeInfoString = "Lisk " + jsonObject["version"] + jsonObject["commit"].substring(0,8) + " Build:(" +jsonObject["build"]+")".replace(/(\r\n\t|\n|\r\t)/gm,"");
       			console.info('Got info');
       			console.info('Commit:'+jsonObject["commit"]);
       			console.info('Commit:'+jsonObject["commit"].substring(0,8));
       			console.info('FORGING:'+process.env.FORGING);
				...
				self.stats.mining = process.env.FORGING;
				...
				var peer_count = jsonObject["meta"]["total"];
